...
export default function AuthLandingPage({ onLogin }) {
  ...
  const handleGoogleSuccess = async (authPayload) => {
    ...
    try {
      ...
      console.error('Backend Google auth request failed:', err);
      setError('Could not connect to backend server for Google login.');
    } finally {
  ...
  const handleDemoGoogleLogin = async () => {
    ...
    try {
      const response = await fetch(apiUrl('/api/auth/google'), {
        ...
        body: JSON.stringify({
          ...
          demoUser: {
            ...
            email: 'student.google@anurag.edu.in',
            name: 'Google Student',
      ...
      if (response.ok && data.success && data.user) {
        ...
        setError(data?.error || 'Google demo authentication failed.');
      }
    } catch (err) {
      console.error('Demo Google login failed:', err);
      setError('Could not connect to backend server for Google demo login.');
    } finally {
      setIsSubmitting(false);
    }
  };
      ...
      {isAuthModalOpen && (
        <div className="auth-modal-backdrop" onClick={() => setIsAuthModalOpen(false)}>
          <div className="auth-modal-card formal-auth-card" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="google-auth-wrapper" style={{ marginBottom: 14 }}>
              <button
                type="button"
